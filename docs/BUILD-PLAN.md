...
| **5 — Itinerary AI Service** | Context builder, prompt templates, OpenAI client, structured output, validator + repair loop, async job lifecycle, quota, cost accounting, resilience (timeout/retry/circuit breaker), demo mode | **The money demo** — one request produces a real validated itinerary saved in Trip Service via Kafka | 100 | 10 |
