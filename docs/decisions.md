...
## ADR-018 — Real-broker Kafka integration tests stand in for a dedicated contract-testing framework
**Chosen:** No Spring Cloud Contract or Pact. Every producer/consumer pair in this system is instead verified by a Testcontainers integration test that runs the real producer, publishes to a real Kafka broker, and asserts the real consumer processes the message correctly — `OutboxToKafkaIT` (auth→generic consumer shape), `UserRegisteredKafkaIT` (auth→user), `ItineraryGenerationKafkaIT` (AI→trip), `RecommendationServiceIT` (user/trip→recommendation).
**Rejected:** Introducing Spring Cloud Contract (stub-jar generation, a contract DSL, Maven plugin wiring) or Pact (a broker, consumer/provider pact files) for the sake of naming the pattern, as the portfolio-build-plan explicitly floated ("write ONE as a demonstration").
**Why:** Both frameworks solve a problem this project doesn't have yet: verifying a contract *without* being able to run the other side. Every service here already runs in CI against every other message shape it depends on, via a real broker — strictly stronger evidence than a stub-based contract test, which only proves the shapes match a recorded expectation, not that a real consumer can process a real message. Bolting on a second framework whose only job is to catch what these tests already catch would be tooling for its own sake — exactly what the portfolio plan's own §2.2 warns against. Real contract-testing tools earn their keep when teams deploy independently and can't spin up the other service in CI; a solo monorepo with Testcontainers doesn't have that constraint. If this system grows into services owned by separate teams, revisit — that's the actual trigger condition, not "portfolios are expected to have one."
